...
| current CI reliability commits | Verify the exported multi-architecture OCI layout directly with runner-provided `tar` and `jq`, so no post-build `apt` repository can invalidate an already successful image build; keep the background-cancellation race test strict about fail-closed state while accepting either legal observation, `CLOSING` or an already completed `LOCKED`; force membership and transparency review fixtures to canonical LF bytes so their frozen Go/Rust digests are identical on Windows, Linux, and macOS |
