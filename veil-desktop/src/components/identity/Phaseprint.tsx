...
// Phaseprint is a presentation-only, non-cryptographic visual checksum. Its
// model must never be used as an identity proof, trust decision, ACL input, or
// replacement for Veil's native fingerprint verification.
...
function normalizedText(
  ...
  if (
    ...
    || new TextEncoder().encode(normalized).length > maxUtf8Bytes
  ) return null;
  return normalized;
}

function normalizedServerOrigin(value: string | null | undefined): string | null {
...
export function resolvePhaseprintSeed(identity: PhaseprintIdentity): ResolvedPhaseprintSeed {
  const identityKey = normalizedText(identity.identityKey, 64);
  if (identityKey && IDENTITY_KEY_RE.test(identityKey) && !/^0{64}$/i.test(identityKey)) {
    return {
      kind: "identity-key",
      canonical: `${PHASEPRINT_DOMAIN}\0identity-key\0${identityKey.toLowerCase()}`,
    };
  }

  const origin = normalizedServerOrigin(identity.canonicalServerOrigin);
  if (!origin) {
    return {
      kind: "anonymous",
      canonical: `${PHASEPRINT_DOMAIN}\0anonymous\0origin-unavailable`,
    };
  }
  const userId = normalizedText(identity.userId, 36)?.toLowerCase() ?? null;
  if (userId && USER_ID_RE.test(userId) && userId !== NIL_USER_ID) {
    return {
      kind: "user-id",
      canonical: `${PHASEPRINT_DOMAIN}\0origin-user-id\0${origin}\0${userId}`,
    };
  }
  ...
  if (technicalUsername) {
    return {
      kind: "username",
      canonical: `${PHASEPRINT_DOMAIN}\0origin-username\0${origin}\0${technicalUsername}`,
    };
  }

  return {
    kind: "anonymous",
    canonical: `${PHASEPRINT_DOMAIN}\0anonymous\0${origin}`,
  };
}
...
export function createPhaseprintModel(identity: PhaseprintIdentity): PhaseprintModel {
  ...
  const seedBytes = new TextEncoder().encode(seed.canonical);
  const words = [
...
export interface PhaseprintProps extends PhaseprintIdentity {
  style?: JSX.CSSProperties;
}

export const Phaseprint: Component<PhaseprintProps> = (props) => {
  const model = createMemo(() => createPhaseprintModel(props));

  return (
    <div
      class="veil-phaseprint"
      data-phaseprint="v1"
      data-phaseprint-seed-kind={model().seedKind}
      style={{
        width: "100%",
        height: "100%",
        overflow: "hidden",
        "border-radius": "inherit",
        background: `linear-gradient(${model().angle}deg, ${model().background}, ${model().wash})`,
        "box-shadow": "inset 0 0 0 1px var(--veil-phaseprint-border)",
        ...(props.style ?? {}),
      }}
    >
      <svg
      ...
      >
        ...
        <circle
          ...
          stroke={model().ink}
          stroke-width="1.4"
          stroke-dasharray={`${model().orbitDash} ${model().orbitGap}`}
          opacity="0.34"
          transform={`rotate(${model().orbitRotation} 32 32)`}
        />
